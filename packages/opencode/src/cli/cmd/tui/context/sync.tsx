            ...
            setStore(
              produce((draft) => {
                ...
                const infos = merged.messages.flatMap((message) => {
                  ...
                  const current = draft.message[sessionID]?.find((item) => item.id === message.id)
                  return current ? [current] : []
                })
                infos.push(
                  ...(draft.message[sessionID] ?? []).filter(
                ...
                for (const message of merged.messages) {
                  ...
                  if (!fetched) {
                    if (merged.parts.get(message.id)) draft.part[message.id] = merged.parts.get(message.id)!
                    continue
                  }
                  const parts = (merged.parts.get(message.id) ?? fetched.parts).flatMap((part) => {
